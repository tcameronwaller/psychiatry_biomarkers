...
if [[ "$report" == "true" ]]; then
  ...
  echo "PubMed: 30239722"
  echo "author: Pulit"
  echo "date: 14 September 2018"
  echo "phenotype: waist-to-hip ratio, adjusted for BMI"
  echo "Human genome version: GRCh37, hg19"
  echo "variant identifier version: dbSNP151"
  echo "----------------------------------------------------------------------"
  echo "----------------------------------------------------------------------"
  echo "----------------------------------------------------------------------"
  echo ""
...
zcat $path_source_file | awk 'BEGIN { FS=" "; OFS=" " } NR > 1 {split($3,a,":"); print a[1], toupper($4), toupper($5), $10, $7, $9}' >> $path_temporary_collection
